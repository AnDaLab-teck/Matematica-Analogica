...
const helpExplanations: Record<Operator, string> = {
  '+': "L'aiuto ti mostrerà i 'salti in avanti' da fare partendo dal primo numero.",
  '-': "L'aiuto ti mostrerà i 'salti all'indietro' da fare partendo dal primo numero.",
  '*': "L'aiuto mostrerà come fare 'salti lunghi' (grandi come il primo numero) per il numero di volte indicato dal secondo.",
  '/': "L'aiuto ti mostrerà quanti 'salti lunghi' (grandi come il divisore) servono per arrivare al numero di partenza.",
};
...
const LineaDel20: React.FC<LineaDel20Props> = ({ selectedOperators, isTestMode, testConfig, onProblemAnswered, problemKey }) => {
  ...
  useEffect(() => {
    ...
    if (isGridView) {
      ...
      jumps.forEach((jump, index) => {
          ...
          const controlY = start.y - Math.min(130, Math.abs(end.x - start.x) * 0.5);
  ...
  const containerClass = layout === 'grid5'
    ...
    : "relative flex flex-nowrap items-center justify-start md:justify-center gap-2 pt-24 pb-4 px-2 overflow-x-auto";


  return (
    <div className={`bg-white/90 backdrop-blur-sm p-4 sm:p-6 rounded-2xl shadow-xl border border-white/50 w-full flex flex-col items-center ${!isComponentActive ? 'pointer-events-none opacity-50' : ''}`}>
      <div className="w-full">
        <h2 className="text-2xl sm:text-3xl font-bold text-center text-slate-700 mb-2">La Linea del 20</h2>
       
        {isHintOn && (
          <div className="bg-yellow-100 border-l-4 border-yellow-400 text-yellow-800 p-3 rounded-md my-4 max-w-2xl mx-auto text-center text-sm sm:text-base flex items-center justify-center gap-3">
            <span className="text-xl">💡</span>
            <p>{helpExplanations[operator]}</p>
          </div>
        )}
