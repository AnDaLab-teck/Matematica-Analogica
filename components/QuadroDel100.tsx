...
const QuadroDel100: React.FC<QuadroDel100Props> = ({ selectedOperators, isTestMode, testConfig, onProblemAnswered, problemKey }) => {
  ...
  return (
    <div className={`bg-white/90 backdrop-blur-sm p-4 sm:p-6 rounded-2xl shadow-xl border border-white/50 w-full flex flex-col items-center ${!isComponentActive ? 'pointer-events-none opacity-50' : ''}`}>
      <h2 className="text-2xl sm:text-3xl font-bold text-center text-slate-700 mb-2">Il Quadro del 100</h2>
      
      {isHintOn && (
        <div className="bg-yellow-100 border-l-4 border-yellow-400 text-yellow-800 p-3 rounded-md my-4 w-full max-w-lg mx-auto text-center text-sm sm:text-base flex items-center justify-center gap-3">
          <span className="text-xl">💡</span>
          <p>{helpExplanations[operator]}</p>
        </div>
      )}

      <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-3 md:gap-8 items-center mt-2">
        
        {/* Main content: Grid of numbers (takes 2/3 of space on lg screens) */}
        <div className="md:col-span-2 w-full mx-auto max-w-lg">
